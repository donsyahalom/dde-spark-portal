...
function getTitleOrder(title) {
  ...
  return idx >= 0 ? idx : TITLE_HIERARCHY.length
}
...
function SparkLogTable({ txnLog, txnLoading, likes, myLikes, isAdmin, currentUser, onLike, onRefresh }) {
  ...
  useEffect(() => {
    if (!isAdmin) return
    Promise.all([
      supabase.from('employees').select('id,first_name,last_name').eq('is_admin',false).eq('is_archived',false).order('last_name'),
...
export default function LeaderboardPage() {
  ...
  const fetchEmployees = async () => {
    const { data } = await supabase.from('employees')
      .select('id, first_name, last_name, vested_sparks, unvested_sparks, redeemed_sparks, job_title, job_grade, is_optional, is_archived')
      .eq('is_admin', false).eq('is_archived', false)
